...
keisenは「┌─┐」のような罫線文字の図をブラウザで書くためのエディタである。等幅のテキストエディタとパレット(細線・太線・角丸の罫線文字と矢印)、箱・表・流れ図のひな形を備え、書いた図はリアルタイムでSVGに変換される。変換では罫線文字をフォントのグリフとしてではなく本物の線として引くため、拡大しても線の太さが揃い、フォントによって罫線が途切れる問題が起きない。線の太さと文字の間隔は書き出し設定で選べ、出力は`currentColor`で描かれるので貼り付け先のライト・ダーク配色にそのまま追従する。
...
エディタに直接書くか、パレットの罫線文字(細線・太線・角丸と矢印)をキャレット位置に挿入する。箱・表・流れ図のひな形から始めて書き換えるのが早い。書き出し設定で線の太さ(細め・標準・太め)と文字の間隔(詰め・標準・広め)を選ぶと、プレビューとコピー・保存するSVGの両方に反映される。
...
- `src/lib/svgconv.ts` — テキストからSVGへの変換(線の太さ・間隔の指定込み)
